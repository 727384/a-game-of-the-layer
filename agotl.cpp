...
void layerreset(int layer)
{
	...
	if (layer == 0)
	{
		...
		else if (phq[3] && !phq[4])
		{
			if (cha[3])
			{
				game.ipoint += pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a)));
				game.total2 += pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a)));
			}
			else
			{
				game.ipoint += pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a)));
				game.total2 += pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a)));
			}
		}
		else
		{			
			if (cha[3])
			{
				game.ipoint += pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a)));
				game.total2 += pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a)));
			}
			else
			{
				game.ipoint += pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a)));
				game.total2 += pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a)));
			}
		}
		game.total1 = 0;
		if (now_cha == 1)
	...
	if (layer == 1)
	{
		...
		game.itimes = 0;
		game.ipoint = 0;
		game.ipmul = (game.total3 >= 500? game.ipmul: (game.total3 >= 50? lBI(128): (game.total3 >= 3? lBI(8): lBI(1))));
...
void printscr(int k, int x)
{
	...
	if (k == 1)
	{
		...
		if (x == 1)
		{
			...
			{                                                                    
				...
				else if (phq[3] && !phq[4])
				{
					if (cha[3])
					{
						cout << ")1 将 Layer 0 转换为 " << pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a))) << " * Layer 1" << endl;
					}
					else
					{
						cout << ")1 将 Layer 0 转换为 " << pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a))) << " * Layer 1" << endl;
					}
				}
				else
				{
					if (cha[3])
					{
						cout << ")1 将 Layer 0 转换为 " << pow(pow(game.ipmul, lBI(1.15)), min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a))) << " * Layer 1" << endl;
					}
					else
					{
						cout << ")1 将 Layer 0 转换为 " << pow(game.ipmul, min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a))) << " * Layer 1" << endl;
					}
				}
			}
		}
		...
		if (x == 3)
		{
			...
			else
			{
				...
				cout << ")1 将 Layer 2 转换为 " << 1 << " * Layer 3" << endl;
			}
		}
		if (x == 4)
		{
	...
	if (k == 3)
	{
		...
		if (x == 2)
		{
			...
			if (cha[4])
			{
				cout << "你有 " << game.layer1a << " * Layer 1a，给 Layer 1 乘数 ^" << min(lBI(2), log10(lBI(10) + lBI(0.001) * game.layer1a));
				if (log10(lBI(10) + lBI(0.001) * game.layer1a) >= 2)
				{
					cout << "（被硬上限）";
				}
				cout << endl;
...
void update(char t, lBI gtick)
{
	...
	if (phq[0] && !phq[4])
	{
		...
		else if (now_cha == 4)
		{
			...
			game.point += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick, pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.total1 += pow(game.s1plus / gtick, lBI(1.1));
			game.total1 += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick, pow(lBI(0.95), game.gtime - cha_sta_tim));
	...
	else if (phq[1])
	{
		...
		{
			game.point += pow(game.s1plus / gtick * pow(game.timetime, lBI(1.25)), pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.point += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick * pow(game.timetime, lBI(1.25)), pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.total1 += pow(game.s1plus / gtick * pow(game.timetime, lBI(1.25)), lBI(1.1));
			game.total1 += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick * pow(game.timetime, lBI(1.25)), pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.s1plus += pow(game.s2plus / gtick * pow(game.timetime, lBI(1.25)), pow(lBI(0.95), game.gtime - cha_sta_tim));
		}
		else if (cha[4])
		{
	...
	else
	{
		...
		{
			game.point += pow(game.s1plus / gtick * game.timetime, pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.point += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick * game.timetime, pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.total1 += pow(game.s1plus / gtick * game.timetime, lBI(1.1));
			game.total1 += pow((pow(lBI(15), game.pointcc) - lBI(1)) / gtick * game.timetime, pow(lBI(0.95), game.gtime - cha_sta_tim));
			game.s1plus += pow(game.s2plus / gtick * game.timetime, pow(lBI(0.95), game.gtime - cha_sta_tim));
		}
		else if (cha[4])
		{
	...
	if (t == '`' && game.point < lBI_Infinity && now_cha != 2)
	{
		...
		else if (now_cha == 4)
		{
			if (game.total3 >= 10000)
			{
				game.point += pow(game.ptplus * game.multi, pow(lBI(0.95), game.gtime - cha_sta_tim));
				game.total1 += pow(game.ptplus * game.multi, pow(lBI(0.95), game.gtime - cha_sta_tim));
			}
			else
			{
				game.point += pow(game.ptplus, pow(lBI(0.95), game.gtime - cha_sta_tim));
				game.total1 += pow(game.ptplus, pow(lBI(0.95), game.gtime - cha_sta_tim));
			}
		}
		else
		{
	...
	if (page == 1)
	{
		if (a == 1)
		{
			...
			else if (t == '4' && game.point < lBI_Infinity && pow(max(lBI({1, 0}), game.point / game.maxpoint), game.rl0mul) > lBI(1) && now_cha != 3)
			{
				layerreset(-1);
			}
			else
