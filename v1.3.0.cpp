...
void buy(int layer, int update)
{
	...
	if (layer == 1)
	{
		...
		if (update == 4 && game.ipoint >= lBI(1000000) && game.autor == 0)
		{
			game.ipoint -= lBI(1000000);
			game.autor = 1;
		}
	}
	if (layer == 2)
...
void layerreset(int layer)
{
	...
	{
		game.point = pow(lBI(4), game.l4u02c - 1) - 1;
		game.multi = max((game.total2 >= 300? game.multi: (game.total2 >= 30? lBI(333): (game.total2 >= 5? lBI(5): lBI(1)))), pow(game.eu03ct - 1, lBI(3)));
		game.ptplus = 1;
		game.ptpcnt = 0;
		game.s1pcnt = 0;
		game.s2pcnt = 0;
		...
		game.ipmul = max((game.total2 >= 500? game.ipmul: (game.total2 >= 50? lBI(128): (game.total2 >= 3? lBI(8): lBI(1)))), pow(game.l4u03c - 1, game.l4u03c - 1));
		game.rl0mul = (game.total2 >= 200? game.rl0mul: (game.total2 >= 20? lBI(25): (game.total2 >= 1? lBI(3): lBI(1))));
		game.l0supd = (game.total2 >= 200? game.l0supd: (game.total2 >= 2? lBI(10): lBI(1)));
		game.imucnt = 0;
		game.su0cnt = 0;
		game.itimes = 0;
		game.etimes ++;
		...
		game.autor = (game.total2 >= 100? game.autor: 0);
	}
	if (layer == 2)
	{
		game.point = pow(lBI(4), game.l4u02c - 1) - 1;
...
void update(char t, lBI gtick)
{
	...
	if (game.point >= lBI_Infinity && game.autor)
	{
		layerreset(0);
	}
	if (game.point / game.maxpoint > lBI(1) && game.autor)
	{
		layerreset(-1);
	}
	...
	if (page == 1)
	{
		if (a == 1)
		{
			...
			else if (t == '4' && game.point < lBI_Infinity && pow(max(lBI({1, 0}), game.point / game.maxpoint), game.rl0mul) > lBI(1))
			{
				layerreset(-1);
			}
			else
	...
	else
	{
		...
		else
		{
			...
			else
			{
				for (int i = 1; i <= 100; i ++)
				{
					update('\0', loadTick * pow(10, floor(log10(lxsj)) - 3) / lxsj);
					cout << "============================" << endl;
					cout << "计算离线时间中...... " << double(i) / 100000 * 100 << "%" << endl;
				}
			}
		}
	}
