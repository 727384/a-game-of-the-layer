...
vector<string> tips = {
		...
		"找不同 （$b 与  ）：              $b     ",
		"ψ(Ω^Ω^Ω^Ω^Ω^...)",
		"360安全管家总给 agotl 报毒，我怀疑是编译器出了问题",
		"修复了 80% 的bug！",
		"这是最后一条新闻"
};
...
void loadgame(string places)
{
	...
	if (x == "1.4.X" || x == "1.4.X_1" || x == "1.4.X_2")
	{
...
void printscr_bi(int k, int x)
{
	...
	if (k == 2)
	{
		...
		{
			cout << "====== 打破\"打破无穷\" ======" << endl;
			cout << ")1 打破\"打破无穷\" 价格：达到 e1.79e308 * Layer 0。" << endl;
		}
		else
		{
			cout << "第 " << x << " 页的这一界面我还没做" << endl;
...
void printscr(int k, int x)
{
	...
	if (k == 3)
	{
		...
		if (x == 5)
		{
			...
			cout << ")h 每秒获得 100% 重置时获得的 Layer 4 （受时间流速影响）价格：1e20 x 当前：" << game.l5dr3 << endl;
	...
	if (!game.break_infinity)
	{
		...
		if (!((phq[0] && !phq[4]) || now_cha == 5))
		{
			...
			{		
				if (game.l5dr4)
				{
					game.l5d3 += pow(pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d2 += pow(pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d1 += pow(pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d0 += pow(pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * pow(game.timetime, lBI(1.25));
				}
				else
				{
					game.l5d3 += pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d2 += pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d1 += pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * pow(game.timetime, lBI(1.25));
					game.l5d0 += pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * pow(game.timetime, lBI(1.25));
				}
				if (game.l5dr3)
				{
					game.l5point += LAYER4MUL / gtick  * pow(game.timetime, lBI(1.25));
					game.total5 += LAYER4MUL / gtick * pow(game.timetime, lBI(1.25));
				}
			}
			else
			{		
				if (game.l5dr4)
				{
					game.l5d3 += pow(pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * game.timetime;
					game.l5d2 += pow(pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * game.timetime;
					game.l5d1 += pow(pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * game.timetime;
					game.l5d0 += pow(pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick * game.timetime;
				}
				else
				{
					game.l5d3 += pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * game.timetime;
					game.l5d2 += pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * game.timetime;
					game.l5d1 += pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * game.timetime;
					game.l5d0 += pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2) / gtick * game.timetime;
				}
				if (game.l5dr3)
				{
					game.l5point += LAYER4MUL / gtick * game.timetime;
					game.total5 += LAYER4MUL / gtick * game.timetime;
				}
			}
		}
		else
		{
			if (game.l5dr4)
			{
				game.l5d3 += pow(pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick;
				game.l5d2 += pow(pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick;
				game.l5d1 += pow(pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick;
				game.l5d0 += pow(pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2), log10(game.total5) * lBI(3)) / gtick;
			}
			else
			{
				game.l5d3 += pow(game.l5d4, lBI(1) + lBI(0.08) * game.l5dr2) / gtick;
				game.l5d2 += pow(game.l5d3, lBI(1) + lBI(0.08) * game.l5dr2) / gtick;
				game.l5d1 += pow(game.l5d2, lBI(1) + lBI(0.08) * game.l5dr2) / gtick;
				game.l5d0 += pow(game.l5d1, lBI(1) + lBI(0.08) * game.l5dr2) / gtick;
			}
			if (game.l5dr3)
			{
				game.l5point += LAYER4MUL / gtick;
				game.total5 += LAYER4MUL / gtick;
			}
		}
		if (game.point >= 1)
		{
