...
#define endl '\n'
#define LAYER4MUL pow(log(log(game.l5d0 + lBI(1)) + lBI(1)), log(log(game.l5d0 + lBI(1)) + lBI(1)))
...
void buy(int layer, int update)
{
	...
	{	
		...
		if (update == 2 && game.l5point >= pow(lBI(10), lBI(1) * (game.l5d2c + lBI(1))))
		{
			game.l5point -= pow(lBI(10), lBI(1) * (game.l5d2c + lBI(1)));
			game.l5d2c ++;
			game.l5d2 ++;
		}		
		if (update == 3 && game.l5point >= pow(lBI(10), lBI(2) * (game.l5d3c + lBI(1))))
		{
			game.l5point -= pow(lBI(10), lBI(2) * (game.l5d3c + lBI(1)));
			game.l5d3c ++;
			game.l5d3 ++;
		}		
		if (update == 4 && game.l5point >= pow(lBI(10), lBI(4) * (game.l5d4c + lBI(1))))
		{
			game.l5point -= pow(lBI(10), lBI(4) * (game.l5d4c + lBI(1)));
			game.l5d4c ++;
			game.l5d4 ++;
		}
	}
...
void printscr(int k, int x)
{
	...
	if (k == 2)
	{
		...
		if (x == 5)
		{
			...
			cout << "Δx = " << game.l5d1 << " ( " << game.l5d1c << " ) 价格：" << game.l5d1c + lBI(1) << endl;
			cout << "ΔΔx = " << game.l5d2 << " ( " << game.l5d2c << " ) 价格：" << pow(lBI(10), lBI(1) * (game.l5d2c + lBI(1))) << endl;
			cout << "ΔΔΔx = " << game.l5d3 << " ( " << game.l5d3c << " ) 价格：" << pow(lBI(10), lBI(2) * (game.l5d3c + lBI(1))) << endl;
			cout << "ΔΔΔΔx = " << game.l5d4 << " ( " << game.l5d4c << " ) 价格：" << pow(lBI(10), lBI(4) * (game.l5d4c + lBI(1))) << endl;
		}
	}
	if (k == 3)
	{
		...
		if (x == 5)
		{
			...
			cout << ")1 改进 Layer 4 乘数公式 价格：" << pow(lBI(3), game.l5dr1) << " x 当前：x 在公式内被视为 x ^ " << game.l5dr1 << endl;
			cout << ")2 x 增量增长速度 ^ 1.08（可叠加） 价格：" << pow(lBI(5), game.l5dr2 * lBI(2)) << " x 当前：^" << pow(lBI(1.08), game.l5dr2) << endl;
			cout << ")3 每秒获得 100% 重置时获得的 Layer 4 价格：1e20 x 当前：" << game.l5dr3 << endl;
			cout << ")4 累积 Layer 4 加成 x 获取 价格：1e55 x 当前：" << game.l5dr4 << endl;
		}
	}
	cout << "============================" << endl;
...
void update(char t, lBI gtick)
{
	...
	game.l5d3 += game.l5d4 * game.timetime / Tick;
	game.l5d2 += game.l5d3 * game.timetime / Tick;
	game.l5d1 += game.l5d2 * game.timetime / Tick;
	game.l5d0 += game.l5d1 * game.timetime / Tick;
	if (game.point >= 1)
	{
		achs[0] = 1;
	}
