...
void loadgame(string places)
{
	...
	if (x == "v1.4.5" || x == "v1.4.6" || x == "v1.4.7" || x == "v1.4.8")
	{
		fin >> game.point;
		fin >> game.ptplus;
		fin >> game.multi;
	...
	if (k == 3)
	{
		...
		if (x == 2)
		{
			...
			if (cha[4])
			{
				...
				{
					cout << "你有 " << game.layer1a << " * Layer 1a，给 Layer 1 乘数 ^" << min(lBI(2), log10(lBI(10) + lBI(0.0001) * game.layer1a));
					if (log10(lBI(10) + lBI(0.0001) * game.layer1a) >= 2)
					{
						cout << "（被硬上限）";
					}
					cout << "（平衡器4）";
